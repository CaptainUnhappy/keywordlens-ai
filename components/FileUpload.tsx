...
interface FileUploadProps {
    onFilesReady: (excelData: string[], imageFile: File, imageData: string, excelFile: File) => void;
}
...
export const FileUpload: React.FC<FileUploadProps> = ({ onFilesReady }) => {
    ...
    const handleStart = async () => {
        if (imageFile && keywords.length > 0 && excelFile) {
            setLoading(true);
            const base64Data = await readFileAsBase64(imageFile);
            onFilesReady(keywords, imageFile, base64Data, excelFile);
        }
    };
